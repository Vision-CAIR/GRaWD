...
class _netG(nn.Module):
    def __init__(self, text_dim=11083, X_dim=3584, rw_config: Config=None):
        super(_netG, self).__init__()
        self.rdc_text = nn.Linear(text_dim, rdc_text_dim)
        self.main = nn.Sequential(
            nn.Linear(z_dim + rdc_text_dim, h_dim),
            nn.LeakyReLU(),
            nn.Linear(h_dim, X_dim),
            nn.Tanh())

        if not rw_config is None and rw_config.rw_params.learnable_alpha:
            self.alpha_raw = nn.Parameter(torch.randn(rw_config.rw_params.num_steps))

    def get_alpha(self) -> Tensor:
        return self.alpha_raw.softmax(dim=0)
